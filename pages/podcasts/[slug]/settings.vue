    ...
    <div class="container">
      ...
      <form v-else @submit.prevent="saveSettings" class="settings-form">
        ...
        <div class="form-section">
          <h2>Webhooks</h2>
          <p class="hint section-hint">
            Notify a chat channel or other system when episode + recording
            events fire. Each webhook can subscribe to one or more events.
            Discord and Slack receive a richly-formatted message;
            <code>generic</code> posts a JSON body that any catcher (n8n,
            Zapier, custom) can consume. Network-level webhooks live on the
            network admin page and fire alongside these.
          </p>
          <WebhookManager :base-url="`/api/podcasts/${podcastSlug}/webhooks`" />
        </div>
...
async function loadAliases() {
  try {
    aliases.value = await $fetch<SlugAlias[]>(`/api/podcasts/${podcastSlug}/aliases`, { headers: ssrHeaders })
  } catch {
    aliases.value = []
  }
}
