...
- `/podcasts/[slug]/episodes/new`, `/podcasts/[slug]/episodes/[id]` — episode CRUD (transcript + chapters file/textarea, per-episode RSS overrides, people attachments, private notes, pull quotes)
...
- `GET/POST /api/podcasts/[slug]/episodes/[id]/pull-quotes`, `PATCH/DELETE .../pull-quotes/[quoteId]`, `POST .../pull-quotes/bulk` — per-episode pull quotes (quote, speaker, timecode, position, `approved`). **Not** in the RSS feed; exposed for static-site builds and inlined via `?include=pull_quotes` on the single-episode GET. **Review gate: both read paths return approved quotes only.** `GET .../pull-quotes?approved=any` is the editor's review queue and the sole opt-out; the `?include=pull_quotes` sync path has none, by design. The default runs that way round on purpose — a caller that forgets the parameter under-exposes, which is visible, while the opposite default would publish unreviewed generated text with nothing erroring. Approval is only settable via `PATCH` (the bulk importer ignores `approved`) and has its own audit actions. The bulk endpoint takes `mode: append|replace` — `replace` is what a transcript job re-run sends so it doesn't stack duplicates. Writes bump `episodes.updated_at` (the incremental-sync staleness signal) and fire `maybeAutoTrigger` **only when the approved payload actually changed** — `syncEpisodeAfterQuoteWrite` in `server/utils/pull-quotes.ts` fingerprints the approved list before and after each write and compares. Most quote writes touch unapproved rows and are invisible downstream, so a rebuild would be pure waste. Comparing payloads rather than reasoning per-field is what makes it exact (reordering an unapproved quote past an approved one rewrites positions but doesn't change the approved list), and any future write path gets it for free. `bumpFeedLastModified` is deliberately never called — the feed is unchanged either way.
...
Calendar output has two surfaces, both rendered by `server/utils/ics.ts`:
- `GET /schedule/[token].ics` — per-user **subscription** feed (podcast or network scope), authenticated by a revocable `schedule_tokens` row. Always includes the recording link.
- `GET /schedule/event/[token].ics` — public **single-event** "add to calendar" download, the link recording webhooks put in their message. The token (`server/utils/event-calendar-token.ts`) is a stateless HMAC over `<episode_id>-<0|1>` keyed on `NUXT_SECRET_KEY`, where the digit is the recording-link disclosure flag. Because the flag is signed, a link posted to a public channel cannot be edited into one that reveals the room URL. Serves only the REC event, never DROP.
...
Tables: `users`, `podcasts` (per-tenant config + soft-delete `status` / `deleted_at`, plus new-episode templates: `episode_title_template`/`episode_description_template`, plus `deploys_paused` kill switch), `podcast_users` (membership), `api_keys`, `api_key_podcasts` (key scope), `episodes` (per-podcast, with `image_url`/`image_filename` for per-episode artwork, plus Podcasting 2.0 fields: `transcript_path`/`transcript_type`, `chapters_url`, `itunes_title`/`itunes_author`/`itunes_explicit`, `season_name`, `episode_display`, `license_identifier`/`license_url`; recording metadata: `recording_starts_at`/`recording_duration_minutes` plus `recording_location_type` (`in_person|remote|mixed`, nullable) and `recording_link` — the link is only stored for remote/mixed, enforced on every write path by `normalizeRecordingLocation` in `server/utils/validate.ts`; status enum is `draft|scheduled|published`; `private_notes` is free-form internal scratch space that never reaches the feed or any public surface — it's listed in `NON_RENDERED_FIELDS` in `episodes/[id].patch.ts` so a notes-only edit doesn't fire a site rebuild, though it still bumps `updated_at` because that column doubles as the human-facing "last modified"), `people` (per-podcast roster: name, photo, href, default role/group, `auto_attach` flag), `episode_people` (many-to-many; `role`/`group` frozen at attach time so historical attribution survives roster edits), `episode_pull_quotes` (per-episode quotable lines: quote, free-text `speaker` — deliberately not an FK to `people` so an off-roster guest still gets attribution — validated `timecode` display string, `position`, and `approved` defaulting to 0; never rendered into the feed, and never leaves Podshelf until approved), `slug_aliases` (permanent record of old podcast slugs; the feed handler matches by alias and emits `<itunes:new-feed-url>`), `podcast_distributions` (per-podcast "Listen on" destinations: name, url, platform_id, notes, position; admin-only metadata, not in feed), `networks` (named groupings of sibling podcasts) + `network_podcasts` (compound-PK join with `position`, allows a podcast in multiple networks), `network_property_definitions` (per-network schema of custom fields: key, label, type ∈ {string|boolean|number|url|color}, required, position) + `network_podcast_properties` (per-(network, podcast) values; FK against `network_podcasts` compound PK so leaving a network auto-clears values), `audit_log` (per-podcast change history, nullable `user_id` for system events like the scheduled-publish flip), `storage_migrations` (queue + progress for end-to-end storage moves), `downloads`, `webhooks` (per-tenant publish + recording webhooks; XOR-scoped to either a podcast or a network via CHECK constraint; `events` is a JSON array of subscribed event names; `include_recording_link` is a per-row disclosure toggle, default 0, gating whether recording webhooks may show the episode's `recording_link` and whether the "add to calendar" link they emit carries it), `corrections` (listener-submitted factual errors; `episode_slug` is stored alongside the nullable `episode_id` FK so a submission still says which episode it meant after an episode delete; `ip_hash` for rate limiting only), `pending_announcements` (publish webhooks parked until the downstream site has actually deployed the episode page — one row per waiting episode, `UNIQUE(episode_id)` so a republish re-arms rather than stacks, `deadline_at` is the give-up point).

Background work: `server/plugins/scheduler.ts` boots an in-process timer that every 60s runs `processScheduledFlips()` (flipping `status='scheduled'` episodes whose `published_at` has arrived), `processPendingPublishes()` (the debounced rebuild dispatch), and `processPendingAnnouncements()` (releasing parked publish webhooks). The feed handler also calls `processScheduledFlips(podcast.id)` on every render as a fallback if the timer is dead. `server/plugins/migration-worker.ts` boots a second timer that picks up pending storage migrations, copies files end-to-end (audio + artwork dirs), then rewrites all matching URLs across `episodes` + `podcasts` and swaps `storage_config_encrypted` to the target. Stale `running` migrations from a dead process are reaped to `failed` on startup. Publish side effects fan out from `firePublishEvent()` in `server/utils/publish-event.ts`: the GitHub `repository_dispatch` fires **immediately** on every go-live (the 15-minute debounce is only for edits to already-published episodes), while the `episode.publish` webhook is handed to `queuePublishAnnouncement()` in `server/utils/announce.ts` rather than sent inline. That announcement links to a page the just-triggered build won't deploy for minutes, so it is held until a HEAD of the episode URL returns 2xx — probed once inline (already-live pages and website-less podcasts send with no added latency), then re-probed by the scheduler each tick, and released regardless after `ANNOUNCE_MAX_WAIT_MINUTES` (30) with a `webhook.publish.deferred.timeout` audit entry. **Do not move the webhook send back inline** — that's the bug this exists to fix. Recording-change events go through `fireRecordingEvent()` in `server/utils/recording-event.ts`; listener corrections go through `fireCorrectionEvent()` in `server/utils/correction-event.ts`. All three call `loadWebhooksForEvent(podcastId, event)` which returns every enabled, subscribed webhook from the podcast itself + any networks it belongs to. Correction payloads are the only ones built from untrusted input — `buildCorrectionBody()` escapes markdown and defuses `@everyone`/`@here` before anything reaches a Discord channel. The event-name + format lists are the single source of truth in `utils/webhook-events.ts` (root `utils/`, so both the Nitro server and the Vue app import it — `server/utils/webhook.ts` re-exports it); the webhook UI's `EVENT_LABELS` is typed `Record<WebhookEvent, string>`, so adding an event without a UI label fails the typecheck rather than silently shipping a webhook the UI can't subscribe to.
...
episodes that already have quotes, `--dry-run` writes nothing. Imported quotes
land **unapproved** and are invisible to site builds until a human approves
them in the editor — `--force` refuses an episode holding approved quotes
(they'd be destroyed by the replace) unless `--force-approved` is passed, and
the gap check reads `?approved=any` so pending candidates don't look like an
empty episode.
